...
class ReflexAgent(Agent):
    ...
    def getAction(self, gameState):
        ...
        newVs = [self.evaluationFunction(gameState, action) for action in legalMoves]
        bestScore = max(newVs)
        bestIndices = [index for index in range(len(newVs)) if newVs[index] == bestScore]
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        shortestScaredTime = sum(newScaredTimes)

        minGhostDistance = 0 * minGhostDistance if shortestScaredTime > 0 else minGhostDistance

        newFoodsDistances = [manhattanDistance(newPos, food) for food in newFood.asList()]
        newNearestFoodDistance = 0 if not newFoodsDistances else min(newFoodsDistances)
        

        return 2 * shortestScaredTime / 10 + (10 / (newNearestFoodDistance + 1)) + (3 * minGhostDistance / 10) + 4 * successorGameState.getScore() / 10

def newVEvaluationFunction(currentGameState):
    """
    This default evaluation function just returns the newV of the state.
    The newV is the same one displayed in the Pacman GUI.
...
class MultiAgentSearchAgent(Agent):
    ...
    def __init__(self, evalFn = 'newVEvaluationFunction', depth = '2'):
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    """
    Your minimax agent with a-b pruning (question 3)
    """
...
def betterEvaluationFunction(currentGameState):
    ...
    "*** YOUR CODE HERE ***"
    minGhostDistance = min([manhattanDistance(pacmanPosition, position) for position in ghostPositions])
    
    shortestScaredTime = sum(scaredTimers)
    ...
    foodsDistances = [manhattanDistance(pacmanPosition, food) for food in foods.asList()]
    nearestFoodDistance = 0 if not foodsDistances else min(foodsDistances)
    

    return capsuleScore + shortestScaredTime + (10 / (nearestFoodDistance + 1)) + (3 * minGhostDistance / 10) + 4 * currentGameState.getScore() / 10
