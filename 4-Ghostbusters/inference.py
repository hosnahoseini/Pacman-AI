...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particle_per_pos = self.numParticles // num_pos
        for position in self.legalPositions:
            for i in range(particle_per_pos):
                self.particles.append(position)
        if self.numParticles % num_pos != 0:
            for i in range(self.numParticles % num_pos):
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            if particle in updated_particles:
                self.particles[i] = updated_particles[particle].sample()
            else:
                dist = self.getPositionDistribution(gameState, particle)
                updated_particles[particle] = dist
...
class JointParticleFilter(ParticleFilter):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            prev_ghost_pos = list(oldParticle)
            for i in range(self.numGhosts):
                if (oldParticle, i) in updated:
                    newParticle[i] = updated[(oldParticle, i)].sample()
                else:
                    new_pos_dist = self.getPositionDistribution(gameState, prev_ghost_pos, i, self.ghostAgents[i])
                    updated[(oldParticle, i)] = new_pos_dist
                    newParticle[i] = new_pos_dist.sample()
            """*** END YOUR CODE HERE ***"""
