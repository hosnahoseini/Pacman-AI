...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            ...
            ghost_queue.push(ghost_position, self.distancer.getDistance(pacmanPosition, ghost_position))
        ...
        for step in [Directions.WEST, Directions.EAST, Directions.NORTH, Directions.SOUTH]:
            try:
                next = self.distancer.getDistance(nearest_ghost, Actions.getSuccessor(pacmanPosition, step))
                if next < current:
                    return step
            except Exception:
                pass
