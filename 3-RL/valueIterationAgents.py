...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next_state, T in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            q_value += T * (R + gamma * self.values[next_state])
        return q_value
    ...
    def computeActionFromValues(self, state):
        ...
        maxIndex = q_values.index(max(q_values))
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 1000):
        """
          ...
          the all_states list. If the chosen state is terminal, nothing
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            turn = i % number_of_states   # start updating all_states from 0 to number_of_states-1, then repeat
            state = all_states[turn]
            ...
            if not self.mdp.isTerminal(state):
                for action in self.mdp.getPossibleActions(state):
                    ...
                    for next_state, prob in T:
                        if not self.mdp.isTerminal(next_state) and prob != 0:
